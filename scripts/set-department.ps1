...
# Sets the department for multiple users via Microsoft Graph $batch (max 20 PATCHes per request)
param(
...
Write-Host "Setze Abteilung '$Department' fuer $($upnList.Count) Benutzer (Graph Batch)..."
    ...
    try {
        ...
        foreach ($r in @($resp.responses)) {
            ...
            if ($statusCode -ge 200 -and $statusCode -lt 300) {
                $updatedUpns.Add($upn)
                Write-Host "Abteilung gesetzt: $upn"
            } else {
                $msg = "HTTP $statusCode"
...
$message = "Abteilung gesetzt: $updated, fehlgeschlagen: $failed"
