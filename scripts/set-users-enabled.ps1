...
# Sets accountEnabled for multiple users via Microsoft Graph $batch (max 20 PATCHes per request)
param(
...
Write-Host "Setze accountEnabled=$enabledBool fuer $($upnList.Count) Benutzer (Graph Batch)..."
...
for ($offset = 0; $offset -lt $upnList.Count; $offset += $batchSize) {
    ...
    for ($i = 0; $i -lt $chunk.Count; $i++) {
        ...
        $requests.Add(@{
            ...
            body    = @{ accountEnabled = $enabledBool }
        })
    }
...
$message = "Aktualisiert: $updated, fehlgeschlagen: $failed"
